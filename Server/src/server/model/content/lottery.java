...
public class lottery {
	...
	public int maximumEntryTimes = 1; //maximum times 1 player can enter per draw
	...
	public void enterLottery(Client c) {
		if (checkEntriesCount(c) < maximumEntryTimes) {
			...
			c.sendMessage("You have already entered once!");
		}
	}
		...
		try {
			...
			for (int j = 0; j < Server.playerHandler.players.length; j++) {
				if (Server.playerHandler.players[j] != null) {
					...
					all.sendMessage("[ <col=2784FF>Lottery </col>] The Lottery has been won by <col=2784FF>" + lotteryPlayersNames.get(winner));
				}
			}
		} catch (Exception e) {
	...
	public void announceFund() {
		...
		for (int j = 0; j < Server.playerHandler.players.length; j++) {
			if (Server.playerHandler.players[j] != null) {
				Client all = (Client)Server.playerHandler.players[j];
				all.sendMessage("[ <col=2784FF>Lottery </col>] The Lottery Fund is currently at <col=2784FF>" + fund + "</col>m.");
			}
		}
	}
