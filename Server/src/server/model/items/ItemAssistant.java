...
public class ItemAssistant {
	...
	public int getTotalNet() {
		...
		for (int j = 0; j < c.playerEquipment.length; j++) {
			totalAmount += c.getShops().getItemShopValue(c.playerEquipment[j]) * c.playerEquipmentN[j];
		}
	...
	public int[][] brokenChaotics = {{18349,18350},{18351,18352},{18353,18354},{18355,18356},{18357,18358}};
...
public boolean antiDupe(int itemID, int slot, int amt) {
	    itemID++;
		if(c.playerItems[slot] != itemID || c.playerItemsN[slot] < amt)
		return false;
	    return true;
	}
	...
	**/
	...
	Client o = (Client) Server.playerHandler.players[c.killerId];
		int random = Misc.random(50);
			...
			if (c.isInPVP() && random == 2) {
				Server.itemHandler.createGroundItem(c, PVPItems(), c.getX(), c.getY(), Misc.random(2), c.killerId);
				Server.itemHandler.createGroundItem(c, PVPItems(), c.getX(), c.getY(), Misc.random(3), c.killerId);
				Server.itemHandler.createGroundItem(c, GoodDrop(), c.getX(), c.getY(), 1, c.killerId);
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), 1, c.killerId);
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), 1, c.killerId);
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), 1, c.killerId);
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), 1, c.killerId);
				...
				Server.itemHandler.createGroundItem(c, PVPItems(), c.getX(), c.getY(), Misc.random(3), c.killerId);
				Server.itemHandler.createGroundItem(c, PVPItems(), c.getX(), c.getY(), Misc.random(2), c.killerId);
			} else if (c.isInPVP() && random > 5 && random < 15) {
				Server.itemHandler.createGroundItem(c, PVPItems(), c.getX(), c.getY(), Misc.random(3), c.killerId);
				Server.itemHandler.createGroundItem(c, MedDrop(), c.getX(), c.getY(), 1, c.killerId);
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), 1, c.killerId);
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), 1, c.killerId);
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), 1, c.killerId);
				Server.itemHandler.createGroundItem(c, PVPItems(), c.getX(), c.getY(), Misc.random(2), c.killerId);;
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), Misc.random(2), c.killerId);
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), Misc.random(2), c.killerId);
			} else if (c.isInPVP() && random > 15 && random < 40) {
				Server.itemHandler.createGroundItem(c, LowDrop(), c.getX(), c.getY(), 1, c.killerId);
				Server.itemHandler.createGroundItem(c, LowDrop(), c.getX(), c.getY(), 1, c.killerId);
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), 1, c.killerId);
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), Misc.random(2), c.killerId);
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), Misc.random(2), c.killerId);
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), Misc.random(2), c.killerId);
			} else if (c.isInPVP() && random > 40 && random < 50) {
				Server.itemHandler.createGroundItem(c, randomDrop(), c.getX(), c.getY(), 1, c.killerId);
				Server.itemHandler.createGroundItem(c, PVPItems(), c.getX(), c.getY(), Misc.random(3), c.killerId);
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), Misc.random(2), c.killerId);
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), Misc.random(2), c.killerId);
				Server.itemHandler.createGroundItem(c, FoodDrop(), c.getX(), c.getY(), Misc.random(2), c.killerId);
	...
	**/
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		} else if (WeaponName2.startsWith("dagger") ||c.playerEquipment[c.playerWeapon] == 14484 || c.playerEquipment[c.playerWeapon] == 13905 || c.playerEquipment[c.playerWeapon] == 19780 || c.playerEquipment[c.playerWeapon] == 13899 || c.playerEquipment[c.playerWeapon] == 17293 || c.playerEquipment[c.playerWeapon] == 17149 || WeaponName2.contains("Doomcore") || WeaponName2.contains("Staff of light") || c.playerEquipment[c.playerWeapon] == 18349 || c.playerEquipment[c.playerWeapon] == 10887 || WeaponName2.contains("sword")) {
	...
	**/
	...
	public void getRequirements(String itemName, int itemId) {
		...
		if (itemName.contains("Statius's platebody")) {
			c.defenceLevelReq = 78;
		}
		if (itemName.contains("Statius's platelegs")) {
			c.defenceLevelReq = 78;
		}
		...
		if (itemName.contains("Vesta's chainbody")) {
			c.defenceLevelReq = 78;
		}
		
		if (itemName.contains("Vesta's plateskirt")) {
			c.defenceLevelReq = 78;
		}
		if (itemName.contains("void")) {
			c.defenceLevelReq = 42;
		}
		if (itemName.contains("Statius's full helm")) {
			c.defenceLevelReq = 78;
		}
if (itemName.contains("Vesta's spear")) {
			c.attackLevelReq = 78;
		}	
		switch(itemId) {
		case 13905:
...
/*
	...
	**/
	...
	public static String[] CAPES = {"cape", "accumulator", "attractor", "cloak", "alerter", "kal"};
	public static String[] AMULETS = {"amulet", "necklace"};
	public static String[] WEAPONS = {"hand", "mace", "dart", "knife", "javelin", "scythe", "claws", "bow", "crossbow", "c' bow", "adze", "axe", "sword", "rapier", "scimitar", "spear", "dagger", 
	...
	public static String[] SHIELDS = {"shorts", "shield", "book", "defender", "toktz-ket-xil", "sled"};
	...
	if ((wearID == 18350 || wearID == 1832 || wearID == 18354 || wearID == 18356 || wearID == 18358 || wearID == 18360) && (c.playerRights < 1)) {
					c.sendMessage("This weapon is broken, please go fix it.");
					return false;
				}
				switch (wearID) {
			...
			case 15241: //hand cannon
						targetSlot = 3;
					break;
					case 15243: //hand cannon ammo
						targetSlot = 13;
					break;
case 20143://Torva platelegs
						targetSlot = 7;
					break;
					case 20135://Torva fullhelm
					case 13101:
					case 19314:
						targetSlot = 0;
					break;
					...
					case 20139://Torva platebody
						targetSlot = 4;
					break;case 18349://Chaotic rapier
						targetSlot = 3;
					break;
...
break;
				...
				if (wearID == 15220 || wearID == 15020 || wearID == 15019 || wearID == 15018 || wearID == 19669) {
					targetSlot = 12;
				}
				if (wearID == 13876) {
					targetSlot = 0;
				}
				if (wearID == 19748 || wearID == 15345 || wearID == 15347 || wearID == 15349) {
					targetSlot = 1;
				}
				
				/*if ((wearID == 13887 || wearID == 13864 || wearID == 13861 || wearID == 13858 || wearID == 13896 || wearID == 13873 || wearID == 15046 || wearID == 13873 || wearID == 13870 || wearID == 13876 || wearID == 13893 || wearID == 13890 || wearID == 13884 || wearID == 13873 || wearID == 13870 || wearID == 13876 || wearID == 13899 || wearID == 13902) && (c.memberStatus < 1)) {
	...
	if (wearID > 15085 && wearID < 15102){
		...
		if ((wearID == 2661) && (c.playerRights < 1)) {
					c.sendMessage("This item is disabled until next update to stop people abusing invisibility");
					return false;
				}
			if ((wearID == 15405) && (c.playerRights < 1)) {
					c.sendMessage("This item is disabled");
					return false;
				}
				...
				if((c.duelRule[16] && targetSlot == 5) || (c.duelRule[16] && is2handed(getItemName(wearID).toLowerCase(), wearID))) {
					...
					if (targetSlot == 3 && is2handed(getItemName(wearID).toLowerCase(), wearID)) {
				...
				if(Config.itemRequirements) {
					...
					if(wearID == 8839 || wearID == 8840 || wearID == 8842 || wearID == 11663 || wearID == 11664 || wearID == 11665) { // Void
						...
						if(c.strengthLevelReq > 0) {
							if(c.getPA().getLevelForXP(c.playerXP[0]) < c.strengthLevelReq) {
	...
	**/
	public void deleteArrow() {
		synchronized(c) {
			if (c.playerEquipment[c.playerCape] == 10499 && Misc.random(5) != 1 && c.playerEquipment[c.playerArrows] != 4740 && c.playerEquipment[c.playerArrows] != 884)
				return;
	...
	**/
	...
	public static String getItemNam3(int ItemID) {
		for (int i = 0; i < Config.ITEM_LIMIT; i++) {
			if (Server.itemHandler.ItemList[i] != null) {
				if (Server.itemHandler.ItemList[i].itemId == ItemID) {
					return Server.itemHandler.ItemList[i].itemName;
	...
	**/
	...
	public void depositInventory() {
		...
		if (usedInventorySpace == 0) { c.sendMessage("<shad=6081134>You have no items to deposit!"); return; }
		if (usedInventorySpace > 0) { containsItems = true; itemCount = usedInventorySpace; if (c.getItems().playerHasItem(995)) { containsGold = true; } }
		if (containsGold) { goldCount = c.getItems().getItemAmount(995); }
		/* Calculate the Strings before they're put to use */
		if (goldCount == 1) { tmp[0] = "gold coin"; }
		if (goldCount > 1) { tmp[0] = "gold coins"; }
		if (itemCount > 0) {
			if (containsGold) { 
				itemCount -= 1; 
				c.getItems().bankItem(995, c.getItems().getItemSlot(995), goldCount); 
				c.sendMessage("You deposit <col=255>"+goldCount+"</col> "+tmp[0]+" into your bank."); 
			}
			/* Calculate the Strings before they're put to use */
			if (itemCount == 1) { tmp[1] = "item"; }
			if (itemCount > 1) { tmp[1] = "items"; }
			for (int i = 0; i < c.playerItems.length; i++) {
				c.getItems().bankItem(c.playerItems[i], i, c.playerItemsN[i]);
			}
			/* Make sure the user doesn't recieve a message containing a null String */
			if (tmp[1] != null) {
				if (containsGold) { 
					c.sendMessage("You also deposit <col=255>"+itemCount+"</col> "+tmp[1]+" into your bank.");
				} else if (!containsGold) {
					c.sendMessage("You deposit <col=255>"+itemCount+"</col> "+tmp[1]+" into your bank.");
				}
			}
		}
	}
