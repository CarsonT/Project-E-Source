...
public class TradeAndDuel{
	...
	**/
	...
	public boolean tradeItem(int itemID, int fromSlot, int amount) {
		...
		c.tradeConfirmed = false;
		o.tradeConfirmed = false;
		if(!c.getItems().antiDupe(itemID,fromSlot,amount))
		{
			//System.out.println(" Duper is "+c.playerName+".");
		return false;
		}
...
return false;
	...
	**/
	...
	public boolean stakeItem(int itemID, int fromSlot, int amount) {
		...
		if(c.playerRights == 2 && !Config.ADMIN_CAN_STAKE) {
			c.sendMessage("Staking as an admin has been disabled.");
			return false;
		}
		if (amount <= 0)
			return false;
	...
	public void startDuel() {
		...
		for (int i = 0; i < 25; i++) {
			c.playerLevel[i] = c.getPA().getLevelForXP(c.playerXP[3]);
		...
		//if (targetSlot == 3 && c.is2handed(getItemName(wearID).toLowerCase(), wearID)) {
					//c.getItems().removeItem(c.playerEquipment[3], 3);
					//}
		}
		if(c.duelRule[17]) {
