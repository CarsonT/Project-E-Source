package server.model.skills.herblore;
...
public class Herblore extends SkillHandler {
    ...
    public static void finishOverload(Client c) {
        if (c.playerLevel[c.playerHerblore] < 96) {
            ...
            c.getPA().sendStatement("You need the herblore level of 96 to make this potion.");
            resetHerblore(c);
            return;
        }
        for (int i = 0; i < OVERLOAD.length; i++) {
        ...
        for (int i = 0; i < OVERLOAD.length; i++) {
            c.getItems().deleteItem(OVERLOAD[i],
            c.getItems().getItemSlot(OVERLOAD[i]), 1);
        }
