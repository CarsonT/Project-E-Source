package server.model.skills.mining;
...
public class Mining {
	...
	private final int[] PICK_REQS = {1,1,6,6,21,31,41,61,85};
	...
	public void startMining(int oreType, int levelReq, int exp) {
		...
		if (goodPick() > 0) {
			if (c.playerLevel[c.playerMining] >= levelReq) {
				...
				resetMining();
				c.getPA().sendStatement("You need a mining level of " + levelReq + " to mine this rock.");
				c.startAnimation(65535);
			}		
		} else {
			resetMining();
			c.getPA().sendStatement("You need a pickaxe to mine this rock.");
			c.startAnimation(65535);
